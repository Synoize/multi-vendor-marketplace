...
CREATE TABLE IF NOT EXISTS vendors (
  ...
  -- Shiprocket Pickup Location (auto-managed, never entered manually in dashboard-side code)
  shiprocket_pickup_id       VARCHAR(100) NULL COMMENT 'Shiprocket pickup location id',
  shiprocket_pickup_code     VARCHAR(100) NULL COMMENT 'Auto-generated unique code, e.g. VENDOR_{id}',
  shiprocket_pickup_location VARCHAR(200) NULL COMMENT 'Shiprocket pickup location code used as pickup_location on create-order',
  shiprocket_pickup_status   ENUM('not_configured','pending','registered','failed') NOT NULL DEFAULT 'not_configured' COMMENT 'Pickup Not Configured / Registration Pending / Registered / Registration Failed',
  shiprocket_pickup_error    TEXT         NULL COMMENT 'Last Shiprocket registration error',
  shiprocket_pickup_pincode  VARCHAR(10)  NULL COMMENT 'Registered pickup pincode snapshot (address-change detection)',
  shiprocket_pickup_address_hash VARCHAR(64) NULL COMMENT 'SHA-256 of registered pickup address (idempotent change detection)',
...
-- ─────────────────────────────────────────────────────────────
-- TABLE: ads_wallets
-- Dedicated advertising wallet per vendor. Funds are added via
-- recharges (Razorpay) and deducted automatically as ads run.
-- ─────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS ads_wallets (
  vendor_id     CHAR(36)        NOT NULL,
  balance       DECIMAL(12,2)   NOT NULL DEFAULT 0.00,
  lifetime_topup DECIMAL(14,2)  NOT NULL DEFAULT 0.00 COMMENT 'total funds ever credited',
  lifetime_spend DECIMAL(14,2)  NOT NULL DEFAULT 0.00 COMMENT 'total funds ever spent on ads',
  updated_at    DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (vendor_id),
  CONSTRAINT fk_ads_wallet_vendor FOREIGN KEY (vendor_id) REFERENCES vendors (id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ─────────────────────────────────────────────────────────────
-- TABLE: ads_wallet_transactions
-- Ledger of every credit (recharge) and debit (campaign spend)
-- against a vendor's ads wallet.
-- ─────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS ads_wallet_transactions (
  id             CHAR(36)        NOT NULL DEFAULT (UUID()),
  vendor_id      CHAR(36)        NOT NULL,
  type           ENUM('credit','debit') NOT NULL,
  amount         DECIMAL(12,2)   NOT NULL,
  description    VARCHAR(255)    NULL,
  reference_type VARCHAR(50)     NULL COMMENT 'recharge | campaign_spend',
  reference_id   VARCHAR(100)    NULL COMMENT 'payment/campaign id',
  created_at     DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  INDEX idx_awt_vendor (vendor_id),
  INDEX idx_awt_type (type),
  INDEX idx_awt_created (created_at),
  CONSTRAINT fk_awt_vendor FOREIGN KEY (vendor_id) REFERENCES vendors (id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- ─────────────────────────────────────────────────────────────
-- TABLE: ads_alert_preferences
-- Per-vendor email alert prefs for the Ads Manager (Settings page).
-- Rows are lazily created with defaults by the API on first read.
-- ─────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS ads_alert_preferences (
  vendor_id                CHAR(36)        NOT NULL,
  notify_budget_exhausted  TINYINT(1)      NOT NULL DEFAULT 1 COMMENT 'Email when a campaign hits its total budget',
  notify_low_balance       TINYINT(1)      NOT NULL DEFAULT 1 COMMENT 'Email when ads wallet drops below threshold',
  low_balance_threshold    DECIMAL(10,2)   NOT NULL DEFAULT 200.00 COMMENT 'Balance (₹) that triggers the low-balance alert',
  max_cpc_bid_limit        DECIMAL(8,2)    NOT NULL DEFAULT 5.00 COMMENT 'Default max CPC bid recommendation',
  last_low_balance_alert_at DATETIME       NULL COMMENT 'Cooldown tracker to avoid alert spam',
  updated_at               DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (vendor_id),
  CONSTRAINT fk_ads_alert_pref_vendor FOREIGN KEY (vendor_id) REFERENCES vendors (id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- Incremental migration for already-created databases (idempotent)
ALTER TABLE vendors ADD COLUMN IF NOT EXISTS fssai_number VARCHAR(14) NULL AFTER gst_number;
ALTER TABLE vendors ADD COLUMN IF NOT EXISTS shiprocket_pickup_id VARCHAR(100) NULL AFTER pickup_pincode;
ALTER TABLE vendors ADD COLUMN IF NOT EXISTS shiprocket_pickup_code VARCHAR(100) NULL AFTER shiprocket_pickup_id;
ALTER TABLE vendors ADD COLUMN IF NOT EXISTS shiprocket_pickup_location VARCHAR(200) NULL AFTER shiprocket_pickup_code;
ALTER TABLE vendors ADD COLUMN IF NOT EXISTS shiprocket_pickup_status ENUM('not_configured','pending','registered','failed') NOT NULL DEFAULT 'not_configured' AFTER shiprocket_pickup_location;
ALTER TABLE vendors ADD COLUMN IF NOT EXISTS shiprocket_pickup_error TEXT NULL AFTER shiprocket_pickup_status;
