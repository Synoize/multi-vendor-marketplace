...
const { globalRateLimit, rateLimit } = require("./middlewares/rateLimit.middleware");
...
// Public platform settings — safe subset for the storefront, sourced from
// database/default-settings.js so the list always matches the known keys.
const { PUBLIC_SETTING_KEYS } = require("./database/default-settings");

app.get(`${API}/settings`, rateLimit("read"), async (req, res, next) => {
  try {
...
// Global safety-net rate limiter for the whole /api/v1 tree.
// Mounted before route routers: every API request is counted here. Health,
// static assets and the settings route above are handled separately.
