...
/**
 * GET /api/v1/shipments/vendor
 * Vendor — list their own shipments (with order + customer info).
 */
const getVendorShipments = asyncHandler(async (req, res) => {
  const { page, limit, offset } = getPagination(req.query);
  const { status } = req.query;

  const conditions = ['s.vendor_id = ?'];
  const params = [req.vendor.id];
  if (status) { conditions.push('s.status = ?'); params.push(status); }
  const where = `WHERE ${conditions.join(' AND ')}`;

  // Live-sync the real Shiprocket status for active shipments on this page
  // (best-effort), then re-fetch so the rows carry the fresh status.
  const pageShipments = await queryRows(
    `SELECT id, awb_code, status FROM shipments s ${where} ORDER BY s.created_at DESC LIMIT ? OFFSET ?`,
    [...params, limit, offset]
  );
  await Promise.all(
    pageShipments
      .filter((s) => s.awb_code && ['processing', 'ready_to_ship', 'shipped', 'in_transit', 'out_for_delivery', 'rto_initiated'].includes(s.status))
      .map((s) => shipmentService.syncShipment(s.id).catch((e) => {}))
  );
  ...
  const [shipments, countRows] = await Promise.all([
    queryRows(
      `SELECT s.*,
        ...
        a.name AS delivery_name, a.phone AS delivery_phone,
        a.line1, a.line2, a.city, a.state, a.pincode,
        (SELECT GROUP_CONCAT(oi.product_name SEPARATOR ', ') FROM order_items oi WHERE oi.order_id = s.order_id AND oi.vendor_id = s.vendor_id) AS product_names
       FROM shipments s
       JOIN orders o ON o.id = s.order_id
