...
/**
 * Create a Razorpay order to recharge a vendor's ads wallet.
 * Does NOT credit anything immediately — funds are credited only after
 * the payment signature is verified (see verifyWalletRecharge).
 */
const createWalletRechargeOrder = async (userId, amount) => {
  const vendor = await queryOne('SELECT id, user_id FROM vendors WHERE user_id = ?', [userId]);
  if (!vendor) throw Object.assign(new Error('Vendor account not found'), { statusCode: 404 });

  const value = Math.round(parseFloat(amount) * 100);
  if (!value || value < 10000) {
    // Minimum ₹100 recharge
    throw Object.assign(new Error('Recharge amount must be at least ₹100'), { statusCode: 400 });
  }

  const rzpOrder = await razorpay.orders.create({
    amount: value,
    currency: 'INR',
    receipt: `ADW${Date.now()}`,
    notes: { userId, vendorId: vendor.id, walletRecharge: '1' },
  });

  await query(
    `INSERT INTO checkout_sessions (user_id, razorpay_order_id, payload, amount, status, expires_at)
     VALUES (?, ?, ?, ?, 'pending', DATE_ADD(NOW(), INTERVAL ${SESSION_TTL_MINUTES} MINUTE))`,
    [userId, rzpOrder.id, JSON.stringify({ type: 'wallet_recharge', vendorId: vendor.id }), value / 100]
  );
...
/**
 * Verify a wallet-recharge payment signature and credit the vendor's ads wallet.
 */
const verifyWalletRecharge = async (razorpayOrderId, razorpayPaymentId, signature, userId) => {
  const expectedSig = crypto
    .createHmac('sha256', config.get('razorpay.keySecret'))
    .update(`${razorpayOrderId}|${razorpayPaymentId}`)
    .digest('hex');
  ...
  if (!session) {
    throw Object.assign(new Error('Recharge session not found. Please try again.'), { statusCode: 400 });
  }
  if (session.status !== 'pending') {
    throw Object.assign(new Error('This recharge has already been processed.'), { statusCode: 400 });
  }
  if (new Date() > new Date(session.expires_at)) {
    await query('UPDATE checkout_sessions SET status = "expired" WHERE id = ?', [session.id]);
    throw Object.assign(new Error('Recharge session has expired. Please try again.'), { statusCode: 400 });
  }

  const payload = typeof session.payload === 'string' ? JSON.parse(session.payload) : session.payload;
  if (payload?.type !== 'wallet_recharge') {
    throw Object.assign(new Error('Invalid recharge session'), { statusCode: 400 });
  }
  ...
  await query("UPDATE checkout_sessions SET status = 'completed' WHERE id = ?", [session.id]);

  return { message: 'Wallet recharged successfully', amount, balance: (await adsService.getWalletWithTransactions(vendor.id)).wallet.balance };
};

module.exports = { initiateRazorpayOrder, createRazorpayOrder, verifyPayment, initiateRefund, handleWebhook, createWalletRechargeOrder, verifyWalletRecharge };
