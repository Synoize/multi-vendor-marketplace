...
async function getCart(userId) {
  try {
    ...
    // for the free-shipping threshold; total shipping is the sum across vendors.
    const vendorMap = {};
    for (const it of items) {
      const vid = it.vendor_id;
      if (!vendorMap[vid]) vendorMap[vid] = { vendorId: vid, storeName: it.vendor_name || 'Vendor', subtotal: 0 };
      vendorMap[vid].subtotal += parseFloat(it.effective_price) * parseInt(it.quantity, 10);
    }
    const shippingBreakdown = Object.values(vendorMap).map((v) => {
      const vendorSubtotal = parseFloat(v.subtotal.toFixed(2));
      const shipping = vendorSubtotal >= freeShippingThreshold ? 0 : shippingCharge;
      return { vendorId: v.vendorId, storeName: v.storeName, subtotal: vendorSubtotal, shipping };
    });
    const shipping = shippingBreakdown.reduce((sum, s) => sum + s.shipping, 0);
