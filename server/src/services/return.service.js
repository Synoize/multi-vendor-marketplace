...
const { initiateRefund } = require('./refund.service');
...
async function requestReturn(orderItemId, userId, data) {
  ...
  const VALID_TYPES = ['return', 'replacement', 'exchange'];
  if (!VALID_TYPES.includes(type)) {
    throw Object.assign(new Error(`Invalid return type. Must be one of: ${VALID_TYPES.join(', ')}`), { statusCode: 400 });
  }
  ...
  if (!item.is_returnable || item.return_type === 'no_return') {
    throw Object.assign(new Error('This product is not returnable'), { statusCode: 400 });
  }

  // Enforce product return_type semantics
  if (item.return_type === 'replacement_only' && type === 'return') {
    throw Object.assign(
      new Error('This product can only be replaced, not returned for a refund'),
      { statusCode: 400 }
    );
  }
  if (item.return_type === 'refund_only' && type !== 'return') {
    throw Object.assign(
      new Error('This product can only be returned for a refund, not exchanged or replaced'),
      { statusCode: 400 }
    );
  }
...
async function updateReturnStatus(returnId, status, adminNotes = null) {
  ...
  // When a refund-type return is completed, issue the refund first so we never
  // mark a return completed without an actual refund record.
  if (status === 'completed' && ret.type === 'return') {
    const item = await queryOne('SELECT * FROM order_items WHERE id = ?', [ret.order_item_id]);
    const payment = await queryOne(
      'SELECT id FROM payments WHERE order_id = ? ORDER BY created_at DESC LIMIT 1',
      [ret.order_id]
    );
    if (!item || !payment) {
      throw Object.assign(new Error('Unable to process refund: order item or payment not found'), { statusCode: 400 });
    }
    await initiateRefund(
      ret.order_id,
      payment.id,
      item.total_price,
      `Refund for return request (${ret.reason || 'Return'}): order item ${item.product_name}`,
      returnId
    );
  }
  ...
  // Keep order item status in sync on completion
  if (status === 'completed') {
    await queryRows(
      "UPDATE order_items SET status = 'returned' WHERE id = ?",
      [ret.order_item_id]
    );
  }
