...
export default function Users() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await api.get(
        `/admin/users?page=${page}&limit=${pageSize}${roleParam}${searchParam}${sortParam}`,
      );
      return res.data;
    },
  });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (val) => (
        <span
          className={`text-xs font-semibold px-2 py-0.5 rounded-full ${val ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"}`}
        >
          {val ? "Active" : "Banned"}
        </span>
      ),
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            View registered customers, vendors, and manage platform access
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <select
            ...
            className="bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-900 outline-none focus:border-secondary-600"
          >
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 outline-none focus:border-secondary-600"
            />
          </div>
        </div>
      </div>
