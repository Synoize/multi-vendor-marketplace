...
export default function VendorDetail() {
  ...
  const defaultCommissionRate = settingsData?.commission_rate;
  ...
  const commissionMutation = useMutation({
    mutationFn: (rate) =>
      api.patch(`/admin/vendors/${id}/commission`, {
        commission_rate: rate,
      }),
    onSuccess: (_, rate) => {
      toast.success(
        rate === "" || rate === null
          ? "Commission reset to platform default"
          : `Commission rate set to ${rate}%`,
      );
      setCommissionDraft("");
      invalidate();
    },
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    <div className="relative">
                      <input
                        ...
                        className="w-24 px-2 py-1 border border-gray-200 rounded-lg text-sm text-gray-900 text-right focus:outline-none focus:border-secondary-800"
                      />
                    ...
                    <button
                      type="button"
                      onClick={() => commissionMutation.mutate(commissionDraft)}
                      disabled={
                        commissionMutation.isPending || commissionDraft === ""
                      }
                      className="px-3 py-1 rounded-lg text-xs font-medium bg-secondary text-secondary-900 hover:bg-secondary-300 disabled:opacity-80"
                    >
                      Set
                    </button>
                    {vendor.commission_rate != null && (
                      <button
                        ...
                        className="px-2 py-1 rounded-lg text-xs font-medium text-gray-400 hover:text-gray-600 hover:bg-gray-100"
                    ...
                    {docs.map((doc, idx) => (
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <a
                            href={`${API_ORIGIN}/api/v1/admin/vendors/${id}/documents/${doc.filename}`}
                            target="_blank"
                            rel="noreferrer"
                            title="View document"
