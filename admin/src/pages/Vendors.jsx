...
export default function Vendors() {
  ...
  const columns = [
      ...
      render: (val, row) => (
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-blue-50 rounded-full flex items-center justify-center border border-blue-100 flex-shrink-0">
            <span className="text-xs font-bold text-blue-500">{val?.charAt(0)?.toUpperCase() || 'S'}</span>
          </div>
          <div>
    ...
    { key: 'email', label: 'Email', render: (val) => <span className="text-gray-500 text-sm">{val || '—'}</span> },
    { key: 'phone', label: 'Phone', render: (val) => <span className="text-gray-500 text-sm">{val || '—'}</span> },
    { key: 'kyc_status', label: 'KYC Status', render: (val) => <StatusBadge status={val || 'not_submitted'} /> },
    { key: 'is_active', label: 'Active', render: (val) => (
      <span className={`text-xs font-semibold ${val ? 'text-emerald-600' : 'text-red-500'}`}>{val ? 'Yes' : 'No'}</span>
    )},
    { key: 'created_at', label: 'Joined', render: (val) => val ? new Date(val).toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: '2-digit' }) : '—' },
  ];
  ...
  return (
    <>
      <Helmet><title>Vendors — Admin</title></Helmet>
      <div className="space-y-5 animate-fadeIn">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-0.5">Manage vendors, KYC approvals, and store settings</p>
          </div>
          <button onClick={() => queryClient.invalidateQueries(['admin-vendors'])} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-all shadow-sm">
            <RefreshCw className={`w-4 h-4 ${isFetching ? 'animate-spin' : ''}`} /> Refresh
          </button>
        </div>

        <div className="bg-white rounded-2xl border border-gray-100 p-4 flex flex-wrap items-center gap-3 shadow-sm">
          <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 flex-1 min-w-[200px] max-w-xs focus-within:border-red-300 focus-within:bg-white transition-all">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
          <div className="p-4 border-b border-gray-100 flex items-center justify-between">
            <p className="text-sm text-gray-500">{isLoading ? 'Loading...' : `${total.toLocaleString()} vendors`}</p>
          </div>
          <DataTable
            columns={columns}
            ...
            renderTopToolbarCustomActions={({ table }) => (
              <button
                onClick={() => handleExport(table.getPrePaginationRowModel().rows.map(r => r.original))}
