...
export default function CampaignDetail() {
  ...
  const handleToggle = (action) => {
    if (actionLock.current || toggleMutation.isPending) return
    actionLock.current = true
    toggleMutation.mutate(action, { onSettled: () => { actionLock.current = false } })
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <button onClick={() => navigate('/campaigns')} className="p-2 hover:bg-secondary-200 rounded-lg">
        ...
        <div className="flex-1">
          ...
          <p className="text-secondary-800 text-sm">Campaign ID: <span className="font-mono">{campaign.id}</span></p>
        </div>
        <div className="flex gap-2">
          {(campaign.status === 'active' || campaign.status === 'paused') && (
            <button
              onClick={() => handleToggle(campaign.status === 'active' ? 'pause' : 'resume')}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <p className="text-secondary-800 text-xs font-semibold uppercase">{card.label}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          {analytics.length === 0 ? (
            <div className="h-60 flex items-center justify-center text-secondary-800 text-sm">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={analytics}>
                  ...
                  <Tooltip />
                  <Line type="monotone" dataKey="clicks" stroke="#9F0202" strokeWidth={2} name="Clicks" />
                  <Line type="monotone" dataKey="impressions" stroke="#F4B400" strokeWidth={2} name="Impressions" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
          ...
          <div className="space-y-3.5 text-sm text-secondary-900">
            <div className="flex justify-between items-center">
              <span className="flex items-center gap-1.5"><Calendar className="h-4 w-4 text-secondary-800" /> Start Date</span>
              <span className="font-semibold text-gray-800">{new Date(campaign.start_date).toLocaleDateString('en-IN')}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="flex items-center gap-1.5"><Calendar className="h-4 w-4 text-secondary-800" /> End Date</span>
              <span className="font-semibold text-gray-800">{new Date(campaign.end_date).toLocaleDateString('en-IN')}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="flex items-center gap-1.5"><Target className="h-4 w-4 text-secondary-800" /> Bid Amount</span>
              <span className="font-semibold text-gray-800 font-mono">₹{campaign.bid_amount} / {campaign.type}</span>
            </div>
            <div className="pt-2 border-t border-gray-50">
              <div className="flex justify-between text-xs mb-1.5">
                <span className="text-secondary-800">Total Spent: ₹{parseFloat(campaign.spent).toFixed(0)}</span>
