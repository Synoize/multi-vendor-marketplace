...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // Never retry the refresh endpoint itself (would recurse).
    ...
    if (error.response?.status === 401 && !originalRequest._retry && !isRefreshRequest) {
      ...
      try {
        ...
        // Only hard-redirect when we're not already on the login page,
        // otherwise a logged-out user would reload /login forever.
