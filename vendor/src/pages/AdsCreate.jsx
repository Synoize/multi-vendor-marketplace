...
import { Megaphone, Search, ArrowLeft, Check } from 'lucide-react'
...
export default function AdsCreate() {
  ...
  const { data: productsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/vendors/products?limit=100&search=${search}`)
  ...
  const createMutation = useMutation({
    mutationFn: async (payload) => {
      return api.post('/ads/vendor', payload)
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        <button onClick={() => navigate('/ads')} className="p-2 hover:bg-gray-100 rounded-lg">
          <ArrowLeft className="h-5 w-5 text-gray-500" />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-gray-900 font-sans">Create Sponsor Campaign</h1>
          <p className="text-gray-500 text-sm">Target customers and boost sales</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">
                {form.type === 'cpc' ? 'Bid Amount (₹ per click) *' : 'Bid Amount (₹ per 1k impressions) *'}
              </label>
              <input
                type="number"
                min="0.5"
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4 flex flex-col h-[400px]">
          <h3 className="font-bold text-gray-900 text-sm border-b border-gray-50 pb-2">Select Products</h3>
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-3 p-2 border rounded-lg cursor-pointer transition-colors ${
                      isSelected ? 'border-[#2874F0] bg-blue-50/50' : 'border-gray-100 hover:bg-gray-50'
                    }`}
                  >
                    <div className="relative h-10 w-10 bg-gray-50 rounded flex-shrink-0 flex items-center justify-center">
                      <img src={p.primary_image || `https://picsum.photos/seed/${p.id}/100`} alt="" className="max-h-full max-w-full object-contain" />
                      {isSelected && (
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-gray-500">₹{p.price} · Stock: {p.stock}</p>
                    </div>
                  </div>
                )
              })}
          ...
          <div className="pt-2 border-t border-gray-50 flex justify-between items-center text-xs">
            <span className="text-gray-500 font-semibold">{selectedProducts.length} selected</span>
            <button
              type="submit"
