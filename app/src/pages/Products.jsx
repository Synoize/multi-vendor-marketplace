...
export default function Products() {
  ...
  // Base query params exclude the cursor so the first page is stable per filter set.
  const baseParams = new URLSearchParams({ ...filters, limit: 20 }).toString();
  ...
  // Accumulate products across cursor pages for a "load more" UX.
  const [allProducts, setAllProducts] = useState([]);
  const [nextCursor, setNextCursor] = useState(null);
  const [hasMore, setHasMore] = useState(false);
  const [loadingMore, setLoadingMore] = useState(false);

  const products = allProducts.length > 0 ? allProducts : data?.products || [];
  ...
  }, [baseParams, data?.nextCursor, data?.hasMore]);
  ...
  const priceRanges = (() => {
    const actualMax = priceStats?.max_price || 0;
    if (actualMax <= 0) return [];

    const max = Math.ceil(actualMax / 100) * 100;
    const rawStep = max / 5;

    // Round step to a nice number (50, 100, 200, 500, 1000, ...)
    ...
    step = Math.max(step, Math.ceil(actualMax / 10 / 50) * 50 || 50);
