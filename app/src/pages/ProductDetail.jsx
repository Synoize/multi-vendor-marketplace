...
export default function ProductDetail() {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const msg =
        ...
        (err?.response?.status === 401 ? "Please login to add to cart" : null);
  ...
  const checkPincode = async () => {
    ...
    try {
      const { data } = await api.get("/products/delivery-check", {
        params: {
          pincode,
          pickup_pincode: product?.vendor_pickup_pincode || undefined,
        },
      });
  ...
  const maxQty = Math.max(1, Math.min(10, Number(availableStock) || 0));
  const clampQtyToStock = () => setQuantity((q) => Math.min(q, maxQty));
  useEffect(() => {
    clampQtyToStock();
  }, [maxQty]);
  const discount =
        ...
        <div className="bg-white px-4 py-3">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="text-white text-xs px-4 border bg-primary rounded-lg hover:bg-opacity-90 transition-colors disabled:opacity-60"
            >
            ...
            >
              {pincodechk.deliverable ? (
                <>
                  ...
                  {pincodechk.place ? ` · ${pincodechk.place}` : ""}
                </>
              ) : (
              ...
              <div className="mb-5">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={checkPincode}
                    disabled={checkingPincode}
                    className="text-white text-sm font-medium px-8 border bg-primary rounded-lg hover:bg-opacity-90 transition-colors disabled:opacity-60"
                  >
                    {checkingPincode ? "Checking..." : "Check"}
                  </button>
                </div>
