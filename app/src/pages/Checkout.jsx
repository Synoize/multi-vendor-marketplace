...
export default function Checkout() {
  ...
  const cart = useCartStore((s) => ({
    ...
    total: s.total,
    shipping: s.shipping,
    shippingBreakdown: s.shippingBreakdown,
    freeShippingThreshold: s.freeShippingThreshold,
    shippingCharge: s.shippingCharge,
  }));

  // COD is only offered when every item in the cart is COD-eligible. If any
  // item disables COD, hide the option and force online payment.
  const codEligible = !cart.items.some(
    (i) => Number(i.is_cod_available) === 0,
  );
  ...
  const handlePlaceOrder = async () => {
    ...
    if (paymentMethod === "cod" && !codEligible) {
      toast.error(
        "Cash on Delivery is not available for one or more items. Please pay online.",
      );
      setPaymentMethod("razorpay");
      return;
    }
    ...
    try {
      ...
      const rzp = new window.Razorpay({
        ...
        // Public logo URL so the store logo is visible in the Razorpay modal.
        image: assets.logo
        ...
        // Prefill name/contact/email so customers don't have to retype them.
        // Falls back to the logged-in user's phone/email when the selected
        // address doesn't carry a phone number.
        prefill: {
          name: selectedAddress.name || authUser?.name || "",
          email: authUser?.email || "",
          contact: selectedAddress?.phone || authUser?.phone || "",
        },
        theme: { color: "#2874F0" },
      });
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="bg-white sm:shadow-sm sm:border sm:border-secondary-200 sm:rounded-xl py-5 sm:px-5">
              ...
              <div className="space-y-3">
                ...
                <button
                  onClick={() => navigate("/profile", { state: { openAddress: true } })}
            ...
            <div className="bg-white sm:shadow-sm sm:border sm:border-secondary-200 sm:rounded-xl sm:py-5 sm:px-5">
              ...
              <div className="grid grid-cols-2 sm:gap-4 gap-3">
                {[
                  ...
                  .filter((opt) => opt.value !== "cod" || codEligible)
                  .map((opt) => {
                  ...
                  <div className="space-y-2.5 text-sm">
                    ...
                    {cart.shippingBreakdown?.length > 1 && (
                      <div className="pl-2 space-y-0.5">
                        {cart.shippingBreakdown.map((s) => (
                          <div
                            key={s.vendorId}
                            className="flex justify-between text-[11px] text-secondary-700"
                          >
                            <span className="truncate pr-2">{s.storeName}</span>
                            <span>
                              {s.shipping === 0
                                ? "FREE"
                                : `₹${s.shipping}`}
