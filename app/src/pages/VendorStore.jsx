...
export default function VendorStore() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      return useVendorStore.getState().fetchVendorStore(vendorId, null);
    },
  });
  ...
  // Reset accumulated products whenever a new store loads
  useEffect(() => {
  ...
  const loadMore = async () => {
    ...
    setLoadingMore(true);
    try {
      const res = await useVendorStore.getState().fetchVendorStore(vendorId, nextCursor);
  ...
  const totalProducts = data?.totalProducts || data?.products?.length || 0;
              ...
              {hasMore && (
                <div className="flex items-center justify-center mt-8">
                  ...
                  >
                    {loadingMore
                      ? "Loading..."
                      : `Load More (${products.length}/${totalProducts || products.length})`}
                  </button>
                </div>
              )}
            </>
